...
class Controls extends FlxActionSet
{
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		keyboardScheme = scheme;
		var uiKeysMap = PlayerSettings.uiKeyBinds;
		var noteKeysMap = PlayerSettings.player1.noteKeyBinds;
		
		#if (haxe >= "4.0.0")
		switch (scheme)
		{
			case Solo:
				inline bindKeys(Control.UI_UP, uiKeysMap.get('ui_up'));
				inline bindKeys(Control.UI_DOWN, uiKeysMap.get('ui_down'));
				inline bindKeys(Control.UI_LEFT, uiKeysMap.get('ui_left'));
				inline bindKeys(Control.UI_RIGHT, uiKeysMap.get('ui_right'));
				inline bindKeys(Control.NOTE_UP, noteKeysMap.get('note_up'));
				inline bindKeys(Control.NOTE_DOWN, noteKeysMap.get('note_down'));
				inline bindKeys(Control.NOTE_LEFT, noteKeysMap.get('note_left'));
				inline bindKeys(Control.NOTE_RIGHT, noteKeysMap.get('note_right'));

				inline bindKeys(Control.ACCEPT, uiKeysMap.get('accept'));
				inline bindKeys(Control.BACK, uiKeysMap.get('back'));
				inline bindKeys(Control.PAUSE, uiKeysMap.get('pause'));
				inline bindKeys(Control.RESET, uiKeysMap.get('reset'));

			// Duo(true) represents "player1" and Duo(false) represents "player2" on the same keyboard
			case Duo(true):
